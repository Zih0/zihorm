  ...
  if (options) {
    ...
    if (where) {
      ...
      conditionKeys.forEach((key, index) => {
        if (isObject(conditionValues[index])) {
          const [[opKey, conditionValue]] = Object.entries(conditionValues[index]);
          conditions += `${key} ${opKey} ${conditionValue} ${index === conditionKeys.length - 1 ? '' : ' AND '}`;
    ...
    if (include) {
      const { fk, joinPk, model } = include;
      query = innerJoin(tableName, fk, model, joinPk, attributes);
    } else {
...
const innerJoin = (tableName, pk, joinTableName, joinPk, attributes = []) => {
  const selection = attributes.length ? attributes.map((s) => `${s}`).join() : '*';
